...
export default function IncomePage() {
  ...
  async function handleSubmit(e) {
    ...
    const {
      data: { user },
    } = await (getSupabase() || { auth: { getUser: async () => ({ data: { user: null } }) } }).auth.getUser();
    if (!user) {
    ...
    if (error) {
      ...
      setToast({ show: true, message: "✓ Ingreso registrado correctamente", type: "success" });
      setAmount("");
      setNote("");
      setCategoryId("");
      setAssociationId("");
  ...
  return (
    <main className={styles.container}>
      ...
      <h1 className={styles.title}>Ingresar ingreso</h1>
